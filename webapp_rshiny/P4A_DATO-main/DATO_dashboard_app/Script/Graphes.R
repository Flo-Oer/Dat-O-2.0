...
#function pour filtrer le dataset sur une periode
...
histogramme <- ggplot(filter_data, aes(x = `Concentration (mg/L)`)) +
  ...
  labs(title = paste("Repartition des Concentrations de Chlore\nPériode :", debut, "à", fin),
...
histogramme <- ggplotly(histogramme, tooltip = c("x", "y"))
...
boxplot <- ggplot(filter_data, aes(x = "", y = `Concentration (mg/L)`)) +
  ...
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"),
        axis.title = element_text(face = "bold", size = 12),  # Increase font size to 12
        axis.text = element_text(size = 10),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.border = element_blank(),
...
boxplot <- ggplotly(boxplot, tooltip = c("x", "y"))
