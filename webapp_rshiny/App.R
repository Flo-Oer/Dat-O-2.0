...
if (!require('StatMatch')) install.packages('StatMatch')
...
source("../prediction_sulfate/scripts/Script pluvio St Martin.R")
source("../prediction_sulfate/scripts/Script reseau neuronnes.R")
  ...
  navbarPage(
    ...
    tabPanel(
      ...
      tabsetPanel(
        ...
        # =====================
        # 7. Prédictions taux de sulfates Flo
        # =====================
        tabPanel(
          "Prédictions taux de sulfates",
          h2("Prédictions taux de sulfates", style = "color:#337ab7;font-weight:bold;"),
          br(),
          tabsetPanel(
            # --- Onglet Vésubie ---
            tabPanel(
              ...
              sidebarLayout(
                sidebarPanel(
                  fileInput(
                    "file_vesubie",
                    "Glisser un fichier csv contenant les données de Vésubie",
                    accept = c(".csv")
                  ),
                  actionButton(
                    "run_vesubie",
                    "Appliquer le modèle de Clément Barcaroli",
                    class = "my-button"
                  ),
                  br(),
                  helpText("Le fichier doit contenir au minimum les colonnes :"),
                  verbatimTextOutput("vesubie_required_cols")
                ),
                mainPanel(
                  h3("Résultats de prédiction", style = "color:#337ab7;"),
                  textOutput("vesubie_resume"),
                  br(),
                  DT::dataTableOutput("vesubie_table")
                )
              )
            ),
            # --- Onglet Joseph Raybaud ---
            tabPanel(
              "Joseph Raybaud",
              sidebarLayout(
                sidebarPanel(
                  fileInput(
                    "file_raybaud",
                    "Glisser un fichier csv contenant les données de Joseph Raybaud",
                    accept = c(".csv")
                  ),
                  actionButton(
                    "run_raybaud",
                  ...
                  br(),
                  helpText("Pour l’instant, ce bouton utilise le même modèle que Vésubie 
                           (à remplacer si un modèle spécifique Raybaud est disponible).")
                ),
                mainPanel(
                  h3("Résultats de prédiction", style = "color:#337ab7;"),
                  textOutput("raybaud_resume"),
                  br(),
                  DT::dataTableOutput("raybaud_table")
                )
              )
            )
          )
...
server <- function(input, output, session) {
  ...
  # Colonnes minimales requises (info UI)
  output$vesubie_required_cols <- renderText({
    if (exists("cols")) {
      paste(cols, collapse = ", ")
    } else {
      "Le vecteur `cols` (variables explicatives du modèle) n’est pas disponible. Vérifiez le script réseau."
    }
  })
  
  # Vésubie : appliquer le modèle de Clément Barcaroli sur un fichier csv
  observeEvent(input$run_vesubie, {
    req(input$file_vesubie)
    
    nouveau_df <- readr::read_csv(input$file_vesubie$datapath)
    nouveau_df <- as.data.frame(nouveau_df)
    
    res <- tryCatch({
      predict_sulfate_vesubie(nouveau_df)
    }, error = function(e) {
      showNotification(
        paste("Erreur lors de la prédiction (Vésubie) :", e$message),
        type = "error"
      )
      NULL
    })
    ...
    output$vesubie_resume <- renderText({
      nb_dep   <- sum(res$depassement_possible, na.rm = TRUE)
      max_pred <- max(res$sulfate_predit, na.rm = TRUE)
      
      if (nb_dep == 0) {
        paste0(
          "Pour ce fichier, le modèle ne détecte aucun risque de dépassement du seuil de 180 mg/L.\n",
          "Concentration maximale prédite : ", max_pred, " mg/L."
        )
      } else {
        paste0(
          nb_dep, " ligne(s) présentent un risque de dépassement (≥ 180 mg/L).\n",
          "Concentration maximale prédite : ", max_pred, " mg/L."
        )
      }
    })
    
    output$vesubie_table <- DT::renderDataTable({
      res %>%
        mutate(
          depassement_possible = ifelse(depassement_possible, "Oui", "Non")
        )
    })
  })
  
  # Joseph Raybaud : pour l'instant, utilise le même modèle que Vésubie
  observeEvent(input$run_raybaud, {
    req(input$file_raybaud)
    
    nouveau_df <- readr::read_csv(input$file_raybaud$datapath)
    nouveau_df <- as.data.frame(nouveau_df)
    
    res <- tryCatch({
      # TODO : remplacer par predict_sulfate_raybaud() si un modèle spécifique est disponible
      predict_sulfate_vesubie(nouveau_df)
    }, error = function(e) {
      showNotification(
        paste("Erreur lors de la prédiction (Joseph Raybaud) :", e$message),
        type = "error"
      )
      NULL
    })
    ...
    output$raybaud_resume <- renderText({
      nb_dep   <- sum(res$depassement_possible, na.rm = TRUE)
      max_pred <- max(res$sulfate_predit, na.rm = TRUE)
    ...
    output$raybaud_table <- DT::renderDataTable({
      res %>%
        mutate(
          depassement_possible = ifelse(depassement_possible, "Oui", "Non")
        )
